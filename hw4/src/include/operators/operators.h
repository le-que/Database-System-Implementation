...
class HashAggregation : public UnaryOperator {
    ...
    std::unordered_map<std::vector<Register>, std::vector<Register>, RegisterVectorHasher> ht;
...
class Intersect : public BinaryOperator {
    ...
    std::vector<Register> output_regs;
    bool seen_input = false;
    std::unordered_map<std::vector<Register>, uint64_t, RegisterVectorHasher> ht;
